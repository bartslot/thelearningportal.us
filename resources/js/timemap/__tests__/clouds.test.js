...
describe('deckDetailFor', () => {
  ...
  it('hands the noise more of the job as the field runs out of pixels', () => {
    ...
    expect(at(2).amount).toBeLessThanOrEqual(at(10).amount)
    expect(at(10).amount).toBeLessThan(at(12).amount)
    expect(at(12).amount).toBeLessThanOrEqual(at(14).amount)
   ...
   * above could tell.
   *
   * So this anchors to a number nobody here chose. The patches are fetched from GIBS at zoom 9 of
   * the standard Web Mercator scheme, whose resolution at the equator is 156543.034 / 2^z metres per
   * pixel — 306 m at z9. That is a property of the tiling scheme, not of our code, and it is what
   * the harvester's own output reports.
   *
   * IT HAS EARNED ITS KEEP TWICE. The source moved z6 -> z8 -> z9 and this test went red both times
   * while every relative assertion above stayed green — exactly the failure it was written to catch,
   * arriving from the direction nobody expects: not a regression, but a genuine improvement silently
   * invalidating a constant.
   *
   * The discriminating case is z8. One screen pixel there covers 611 m, still twice a source texel,
   * so the source has NOT run out and the noise must be given nothing extra. Under the previous z8
   * constant the same point reads as exactly one texel per pixel and the noise starts lifting — so
   * this goes red on the old value rather than merely passing on the new one.
   */
  it('judges the source run out at 306 m per texel, which is GIBS zoom 9', () => {
    const WEB_MERCATOR_M_PER_PIXEL_Z0 = 156543.03392
    const SOURCE_ZOOM = 9
    const sourceMetresPerTexel = WEB_MERCATOR_M_PER_PIXEL_Z0 / 2 ** SOURCE_ZOOM
    expect(sourceMetresPerTexel).toBeCloseTo(306, 0)
    ...
    expect(at(8).amount).toBeCloseTo(0.14, 6)
