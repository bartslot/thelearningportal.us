const k="#0f172a",g="#f59e0b",M=["A","B","C","D"],T=["#e11d48","#0284c7","#d97706","#059669"];const z=["Nice!","Great!","Perfect!","Brilliant!","On fire!"],q=["Almost!","Good try!","Keep going!"];let S=!1;function L(){if(S)return;S=!0;const $=document.createElement("style");$.textContent=`
  ...
  `,document.head.appendChild($)}class v{constructor(t){this.host=t,this._questions=[],this._index=0,this._answered=new Map,this._score=0,this._streak=0,this._onComplete=null,this._display=[],this._gateUntil=new Map,this._openedAt=new Map,this._responses=[],this._focusDrops=0,this._gateTimer=null,this._onVisibility=null}static _shuffledIndices(t){const s=Array.from({length:t},(e,l)=>l);for(let e=s.length-1;e>0;e--){const l=Math.floor(Math.random()*(e+1));[s[e],s[l]]=[s[l],s[e]]}return s}static _seededShuffle(t,s){let e=s>>>0;const l=()=>{e=e+1831565813>>>0;let n=Math.imul(e^e>>>15,1|e);return n=n+Math.imul(n^n>>>7,61|n)^n,((n^n>>>14)>>>0)/4294967296},o=Array.from({length:t},(n,i)=>i);for(let n=o.length-1;n>0;n--){const i=Math.floor(l()*(n+1));[o[n],o[i]]=[o[i],o[n]]}return o}static _readGateMs(t){const s=String(t.question||"")+(t.options||[]).join("");return Math.min(7e3,2e3+Math.round(s.length*55/10))}get isVisible(){return this._questions.length>0}show({questions:t,onComplete:s=null,submitUrl:e=null,leaderboardUrl:l=null,hasClassroom:o=!1,shuffleMode:n="per_player"}){if(L(),this._questions=Array.isArray(t)?t.filter(i=>i?.question):[],this._index=0,this._answered=new Map,this._score=0,this._streak=0,this._onComplete=s,this._submitUrl=e,this._leaderboardUrl=l,this._hasClassroom=o,this._classCode=(()=>{try{return localStorage.getItem("lp_class_code")||""}catch{return""}})(),!this._questions.length){this.hide();return}this._shuffleSalt=(t?.[0]?.question||"").split("").reduce((i,a)=>i*31+a.charCodeAt(0)>>>0,7),this._display=this._questions.map((i,a)=>{const r=(i.options||[]).length||4;return n==="off"?Array.from({length:r},(d,u)=>u):n==="once"?v._seededShuffle(r,a+1+this._shuffleSalt):v._shuffledIndices(r)}),this._gateUntil=new Map,this._openedAt=new Map,this._responses=[],this._focusDrops=0,this._onVisibility=()=>{document.hidden&&this.isVisible&&this._showFocusVeil()},document.addEventListener("visibilitychange",this._onVisibility),this.host.style.pointerEvents="auto",this._render()}hide(){this._questions=[],this.host.innerHTML="",this.host.style.pointerEvents="none",this._gateTimer&&(clearTimeout(this._gateTimer),this._gateTimer=null),this._onVisibility&&(document.removeEventListener("visibilitychange",this._onVisibility),this._onVisibility=null)}_showFocusVeil(){if(this._focusDrops++,this.host.querySelector("[data-focus-veil]"))return;const t=document.createElement("div");t.dataset.focusVeil="1",t.style.cssText=`position:absolute; inset:0; z-index:20; display:flex; flex-direction:column;
      ...
      <div style="font-size:15px; color:#94a3b8;">Stay with the story — tap to continue.</div>`,t.addEventListener("click",()=>t.remove()),this.host.firstElementChild?.appendChild(t)||this.host.appendChild(t)}_integritySummary(){const t=this._responses.map(i=>i.ms).filter(i=>i>=0),s=t.length?Math.round(t.reduce((i,a)=>i+a,0)/t.length):0,e=t.filter(i=>i<2e3).length;let l=0,o=0,n=null;for(const i of this._responses)o=i.displayIndex===n?o+1:1,n=i.displayIndex,l=Math.max(l,o);return{avg_ms:s,rapid_guesses:e,same_letter_streak:l,focus_drops:this._focusDrops}}_correctCount(){let t=0;return this._answered.forEach((s,e)=>{(this._display[e]||[])[s]===Number(this._questions[e]?.correct_index)&&t++}),t}_render(t=null){const s=this._questions[this._index];if(!s)return;const e=this._questions.length,l=this._answered.get(this._index),o=l!==void 0,n=this._display[this._index]||(s.options||[]).map((p,c)=>c),i=o&&n[l]===Number(s.correct_index);!o&&!this._gateUntil.has(this._index)&&this._gateUntil.set(this._index,performance.now()+v._readGateMs(s));const a=o?0:Math.max(0,(this._gateUntil.get(this._index)??0)-performance.now()),r=a>50;!r&&!o&&!this._openedAt.has(this._index)&&this._openedAt.set(this._index,performance.now()),r&&(this._gateTimer&&clearTimeout(this._gateTimer),this._gateTimer=setTimeout(()=>this._render(),Math.min(a+30,500)));const u=n.map(p=>(s.options||[])[p]).slice(0,4).map((p,c)=>{const h=n[c]===Number(s.correct_index);let f="rgba(255,255,255,0.06)",m="rgba(255,255,255,0.12)",x="";return o&&h?(f="rgba(16,185,129,0.28)",m="#10b981",(t?.kind==="correct"&&c===l||t?.kind==="wrong")&&(x="qz-correct")):o&&c===l&&!h&&(f="rgba(225,29,72,0.22)",m="#e11d48",t?.kind==="wrong"&&(x="qz-wrong")),`
        <button data-opt="${c}" ${o||r?"disabled":""} class="${x}"
          ...
          <span style="display:inline-flex; align-items:center; justify-content:center; width:28px; height:28px;
                       ...
                       background:${T[c]};">${M[c]}</span>
          <span>${this._escape(p)}</span>
        </button>`}).join("");let _="";if(o){const p=t?.word??(i?s.asks_ahead?"You already knew this!":z[this._index%z.length]:s.asks_ahead?"No worries — you'll hear this later in the story!":q[this._index%q.length]);_=`
      ...
      </div>`,this.host.querySelectorAll("[data-opt]").forEach(p=>{p.addEventListener("click",()=>this._answer(Number(p.dataset.opt),p))}),this.host.querySelector("[data-prev]")?.addEventListener("click",()=>{this._index>0&&(this._index--,this._render())}),this.host.querySelector("[data-next]")?.addEventListener("click",()=>{this._index<e-1?(this._index++,this._render()):this._renderScoreScreen()}),t?.kind==="correct"&&this._playCorrectEffects(t)}_answer(t,s){if(this._answered.has(this._index)||(this._gateUntil.get(this._index)??0)>performance.now()+50)return;const e=this._questions[this._index],l=this._display[this._index]||(e.options||[]).map((i,a)=>a),o=l[t]===Number(e.correct_index);this._answered.set(this._index,t);const n=this._openedAt.get(this._index);if(this._responses.push({ms:n!==void 0?Math.round(performance.now()-n):-1,displayIndex:t,snapshot:{question_order:this._index+1,question_text:String(e.question||""),chosen_text:String((e.options||[])[l[t]]??""),correct_text:String((e.options||[])[Number(e.correct_index)]??""),was_correct:o,response_ms:n!==void 0?Math.round(performance.now()-n):null,asks_ahead:!!e.asks_ahead}}),o){this._streak++;const a=10+(this._streak>=3?5:0),r=this._score;this._score+=a;const d=s.getBoundingClientRect();this._render({kind:"correct",gained:a,from:r,at:{x:d.left+d.width/2,y:d.top}})}else e.asks_ahead||(this._streak=0),this._render({kind:"wrong"})}_playCorrectEffects({gained:t,from:s,at:e}){const l=this.host.querySelector("[data-score]"),o=this.host.querySelector("[data-score-value]");if(l&&o){l.classList.add("qz-score-bump");const i=this._score,a=performance.now(),r=d=>{const u=Math.min(1,(d-a)/500);o.textContent=Math.round(s+(i-s)*(1-Math.pow(1-u,3))),u<1&&requestAnimationFrame(r)};requestAnimationFrame(r)}const n=document.createElement("div");n.textContent=`+${t}`,n.style.cssText=`position:fixed; left:${e.x}px; top:${e.y}px; z-index:90; pointer-events:none;
      transform:translateX(-50%); font-weight:900; font-size:26px; color:#fbbf24;
      text-shadow:0 2px 10px rgba(0,0,0,0.6); animation: qz-float-up 0.9s ease-out forwards;`,document.body.appendChild(n),setTimeout(()=>n.remove(),950);for(let i=0;i<10;i++){const a=document.createElement("div"),r=Math.PI*2*i/10+Math.random()*.5,d=46+Math.random()*34,u=["#fbbf24","#34d399","#38bdf8","#f472b6"];a.style.cssText=`position:fixed; left:${e.x}px; top:${e.y}px; z-index:89; pointer-events:none;
      ...
      <div style="position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
        ...
        </div>`}).join(""),i=e!==null&&e>t.length?`<div style="margin-top:10px; font-size:14px; color:#fbbf24; font-weight:700;">You're #${e} of ${s} — keep climbing!</div>`:"";this.host.innerHTML=`
      <div style="position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
                  background:rgba(2,6,23,0.72); backdrop-filter:blur(6px);">
        <div class="qz-card" style="width:min(520px, calc(100vw - 32px)); max-height:calc(100vh - 60px); overflow-y:auto;
          ...
          <div style="display:flex; flex-direction:column; gap:4px; text-align:left;">${n||'<span style="color:#64748b;">No scores yet — you could be first!</span>'}</div>
          ${i}
          <button data-done
