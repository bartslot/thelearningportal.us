...
/**
 * The narrators we actually ship, as opposed to the demo cast AvatarSeeder invents.
 *
 * AvatarSeeder generates a roster with made-up names and de-duplicates slugs in a loop, so running
 * it twice produces "ron-slot-2". That is fine for filling a picker on a fresh database and useless
 * for a real person who has to exist, once, with the same slug, on every environment.
 *
...
class NarratorSeeder extends Seeder
{
    ...
    public function run(): void
    {
        foreach (self::NARRATORS as $narrator) {
            $avatar = Avatar::withTrashed()->firstWhere('slug', $narrator['slug']);
            ...
            $avatar = Avatar::updateOrCreate(
                ['slug' => $narrator['slug']],
                $narrator + ['sort_order' => $avatar?->sort_order ?? ((int) Avatar::max('sort_order') + 1)],
            );

            foreach (['portrait_path', 'intro_video_path'] as $field) {
                $path = $narrator[$field];
                if ($path && ! Storage::disk('public')->exists($path)) {
                    $this->command?->warn("{$avatar->name}: {$field} points at {$path}, which is not on this disk yet — run scripts/sync-narrator-media.sh.");
                }
            }

            $this->command?->info("{$avatar->name} ({$avatar->slug}) — {$avatar->voice_provider} {$avatar->voice_id}");
        }
    }
}
